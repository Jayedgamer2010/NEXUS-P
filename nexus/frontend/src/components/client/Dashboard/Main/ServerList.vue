...
const timeStatuses = ref<Record<string, { status: string; queue_position: number | null; minutes_remaining: number }>>({});
...
const fetchServers = async () => {
    try {
        ...
        // Fetch time status for each server
        for (const server of servers.value) {
            if (server.suspended || server.identifier) {
                try {
                    const timeData = await TimeSystem.getTimeStatus(server.identifier);
                    if (timeData.success) {
                        timeStatuses.value[server.identifier] = {
                            status: timeData.time_status.status,
                            queue_position: timeData.time_status.queue_position,
                            minutes_remaining: timeData.time_status.minutes_remaining,
                        };
                    }
                } catch {
                    timeStatuses.value[server.identifier] = { status: 'unknown', queue_position: null, minutes_remaining: 0 };
                }
            }
        }
    } catch (error) {
...
const getTimeBadge = (server: Server | QueuedServer): { label: string; color: string; icon: string } | null => {
    if ('status' in server && (server.status === 'pending' || server.status === 'building')) {
        return null;
    }
    const svId = 'identifier' in server ? server.identifier : null;
    if (!svId || !timeStatuses.value[svId]) return null;

    const ts = timeStatuses.value[svId];
    switch (ts.status) {
        case 'active':
            return { label: `${ts.minutes_remaining}m`, color: 'bg-green-500/20 text-green-400', icon: 'clock' };
        case 'queued':
            return { label: `#${ts.queue_position}`, color: 'bg-yellow-500/20 text-yellow-400', icon: 'queue' };
        case 'cooldown':
            return { label: 'Cooling', color: 'bg-orange-500/20 text-orange-400', icon: 'cooldown' };
        case 'suspended':
            return { label: 'No time', color: 'bg-gray-500/20 text-gray-400', icon: 'suspended' };
        default:
            return null;
    }
};
